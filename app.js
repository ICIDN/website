/* Indian Electronic Literature Atlas: application script.
   Data: assets/data/atlas-data.js (window.IEL_DATA), assets/data/india-states.js (window.IEL_STATES).
   Settings: assets/js/config.js (window.IEL_CONFIG). */
(function(){
"use strict";
...
let layer='works', selected=null, base=CFG.defaultBase||'sat';
...
const bases={
  ...
  street:L.tileLayer(CFG.streetTiles||'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',
      {maxZoom:19,subdomains:'abc',attribution:'&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'}),
  offline:L.imageOverlay('assets/img/india-satellite-offline.jpg',indiaBounds,
...
function styleFor(f){
  ...
  const onImage=base!=='street';
  return { color: sel ? '#ffffff' : (onImage?'rgba(255,255,255,.8)':'#5B6072'), weight: sel?3:1,
    fillColor: c||'#ffffff', fillOpacity: c ? (onImage?.6:.75) : (onImage?.03:.12) };
}
...
// Optional: send entries straight to the editors (set in assets/js/config.js).
