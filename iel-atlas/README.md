...
All files live in one folder, `projects/iel-atlas/`, with no subfolders:

```
index.html                  Page markup (all sections)
style.css                   Styling, light and dark themes
config.js                   Settings you edit: map tiles, MapTiler key, submission email/endpoint
app.js                      Application logic (map, clusters, search, filters, timeline, dialogs)
atlas-data.js               Works + scholarship + scholars (window.IEL_DATA)
india-states.js             State boundaries with label points (window.IEL_STATES)
logo-icidn.png, card.jpg    ICIDN logo and the Projects-page tile image
india-satellite-offline.jpg Built-in NASA image, used if live map tiles fail
leaflet.js, leaflet.css, leaflet.markercluster.js, MarkerCluster.css, MarkerCluster.Default.css
                            Map and clustering libraries
```
...
optional place-name labels, and a built-in NASA image as an automatic fallback. Change providers in `config.js`.
...
`atlas-data.js` holds plain JSON after `window.IEL_DATA =`. Each work has fields such as `title`, `author`,
